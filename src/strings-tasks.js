...
function sumOfCodes(str) {
  ...
  return array.reduce((sum, character) => sum + character.charCodeAt(0), 0);
}
...
function isPalindrome(str) {
  const forwardStr = str.replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
...
function findLongestWord(sentence) {
  ...
  const longestWord = words.sort(
    (shortest, longest) => longest.length - shortest.length
  )[0];
  return longestWord;
}
...
function encodeToRot13(str) {
  return str
    ...
    .map((char) => {
      if (/[a-zA-Z]/.test(char)) {
        const charStartCode = char === char.toUpperCase() ? 65 : 97;
        return String.fromCharCode(
          ((char.charCodeAt(0) - charStartCode + 13) % 26) + charStartCode
        );
      }
      return char;
    })
